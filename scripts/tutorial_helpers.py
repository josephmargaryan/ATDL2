...
def plot_weight_scatter(weights_before, weights_after, sample=20000, xlim=None, ylim=None, save=None):
    """
...
def plot_weight_histogram(weights, title="Weight Distribution", log_scale=False, save=None, bins=200):
    """
    ...
    # Create histogram
    counts, edges, patches = ax.hist(w_flat, bins=bins, color='green', alpha=0.7, edgecolor='black')
...
def plot_mixture_components(prior, xlim=(-1, 1), save=None):
    ...
    for j in range(len(mu)):
        # Gaussian PDF
        gauss = (1 / (sigma[j] * np.sqrt(2 * np.pi))) * np.exp(-0.5 * ((x - mu[j]) / sigma[j])**2)
        # Scale by mixing proportion
        y = pi[j] * gauss

        if j == 0:
            # Zero component - highlight differently
            ax.plot(x, y, 'r-', linewidth=2, alpha=0.8, label=f'Component 0 (zero spike): π={pi[j]:.3f}')
            ax.axvline(mu[j], color='r', linestyle='--', linewidth=1, alpha=0.5)
        else:
            ax.plot(x, y, linewidth=1.5, alpha=0.7, label=f'Component {j}: μ={mu[j]:.3f}, π={pi[j]:.4f}')
...
def plot_comparison_histograms(weights_pre, weights_retrained, weights_quantized, save_prefix=None):
    """
    ...
    for col, (weights, title) in enumerate(zip(weights_list, titles)):
        ...
        axes[0, col].text(0.02, 0.98, stats_text, transform=axes[0, col].transAxes,
                         verticalalignment='top', bbox=dict(boxstyle='round', facecolor='wheat', alpha=0.5),
                         fontsize=9)
