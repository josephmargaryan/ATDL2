...
class BasicBlock(nn.Module):
    def __init__(self, in_planes, out_planes, stride):
        ...
        self.conv1 = nn.Conv2d(
            in_planes, out_planes, 3, stride=stride, padding=1, bias=False
        )
        self.bn2 = nn.BatchNorm2d(out_planes)
        self.conv2 = nn.Conv2d(
            out_planes, out_planes, 3, stride=1, padding=1, bias=False
        )
        self.shortcut = None
...
class NetworkBlock(nn.Module):
    def __init__(self, n_layers, in_planes, out_planes, stride):
        ...
        layers = [
            BasicBlock(
                in_planes if i == 0 else out_planes, out_planes, stride if i == 0 else 1
            )
