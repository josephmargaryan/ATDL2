...
def make_loaders(dataset: str, batch_size: int, num_workers: int) -> Tuple[DataLoader, DataLoader, int]:
    if dataset == "mnist":
        ...
        train = datasets.MNIST(root="./data", train=True, download=True, transform=tf_train)
        test  = datasets.MNIST(root="./data", train=False, download=True, transform=tf_test)
        return (DataLoader(train, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=True),
                DataLoader(test,  batch_size=2*batch_size, shuffle=False, num_workers=num_workers, pin_memory=True),
                10)
