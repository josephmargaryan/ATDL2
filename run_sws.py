...
import torch
from sws.utils import set_seed, get_device, ensure_dir, collect_env, collect_weight_params
...
def main():
    ...
    ap.add_argument("--init-means", choices=["from_weights","fixed"], default="from_weights")
    ...
    # Hyper-priors
    ap.add_argument("--gamma-alpha", type=float, default=None, help="Gamma prior α for non-zero comps")
    ap.add_argument("--gamma-beta", type=float, default=None,  help="Gamma prior β for non-zero comps")
    ap.add_argument("--gamma-alpha-zero", type=float, default=None, help="Gamma prior α for zero comp")
    ap.add_argument("--gamma-beta-zero", type=float, default=None,  help="Gamma prior β for zero comp")
    ...
    ap.add_argument("--paper-defaults", action="store_true",
    ...
    if args.paper-defaults:
        ...
        # Gamma priors close to tutorial values (zero stronger than others)
        if args.gamma_alpha is None:      args.gamma_alpha = 250.0
        if args.gamma_beta  is None:      args.gamma_beta  = 0.1
        if args.gamma_alpha_zero is None: args.gamma_alpha_zero = 5e3
        if args.gamma_beta_zero  is None: args.gamma_beta_zero  = 2.0
        # Very strict merge threshold (their code uses ~1e-10)
    ...
    prior: MixturePrior = init_mixture(
        model, J=args.num_components, pi0=args.pi0,
    ...
    re_acc = retrain_soft_weight_sharing(
        ...
        cr_kwargs={"use_huffman": not args.no_huffman,
